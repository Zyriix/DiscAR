...
class Evaluator:
    ...
    def read_activations(self, npz_path: Union[str, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        if isinstance(npz_path, str):
            ...
            #
            # 支持两种 npz：
            # 1) 图像 npz：包含 'arr_0'，这里正常读取图像并计算 activations；
            # 2) 统计量 npz：只包含 'mu' / 'sigma' 等，没有 'arr_0'，
            #    这时不需要 activations，直接返回占位数组，后续 read_statistics
            #    会从文件里的统计量读取，不会用到这些 activations。
            with np.load(npz_path) as obj:
                keys = set(obj.files)

            # stats-only npz：让上层通过 read_statistics 直接用 mu/sigma
            if "mu" in keys and "sigma" in keys and "arr_0" not in keys:
                dummy = np.zeros((0, 1), dtype=np.float32)
                return dummy, dummy

            # image npz：需要存在 'arr_0'
